...
//  SmartBuildingSite
//
//  Created by XT Xiong on 2017/10/18.
//  Copyright © 2017年 iSoftStone. All rights reserved.
//

#import <Foundation/Foundation.h>
#import "APIURLResponse.h"
#import "CachePolicy.h"

#define kCarouseChannelClientId  @"1031"    //  app从直播轮播台申请的客户端id
...
typedef NS_ENUM(NSUInteger, APIManagerErrorType) {
    ...
    APIManagerErrorTypeNoData,          //  没有数据
    APIManagerErrorTypeNoLogin          //  未登录。Token 失效
};
 ...
 *  获取缓存数据
 *
 *  @param manager manager 自身
 *  @param object  object 缓存数据
 */
- (void)managerLoadLoadDataDidSuccess:(APIBaseManager *)manager andWithLocalData:(id)object;
...
@protocol APIManagerVaildator <NSObject>
...
/*                               APIManager                                                      */
/*************************************************************************************************/
//APIBaseManager的派生类必须符合这些protocol
...
@optional
- (void)manager:(APIBaseManager *)manager beforePerformSuccessWithResponse:(APIURLResponse *)response;
- (void)manager:(APIBaseManager *)manager afterPerformSuccessWithResponse:(APIURLResponse *)response;

- (void)manager:(APIBaseManager *)manager beforePerformFailWithResponse:(APIURLResponse *)response;
- (void)manager:(APIBaseManager *)manager afterPerformFailWithResponse:(APIURLResponse *)response;
...
@property (nonatomic, weak) id <APIManagerVaildator> validator;
@property (nonatomic, weak) NSObject <APIManager> *child;
@property (nonatomic, weak) id <APIManagerInterceptor> interceptor;
@property (nonatomic, assign) BOOL shouldCleanData; //  是否清除数据
@property (nonatomic, assign) BOOL shouldParse;     //  是否分页请求
@property (nonatomic, assign) BOOL hadGotAllData;     //获得所有数据
//@property (nonatomic, strong) NSString *requestUrlString;     //请求Url
@property (nonatomic, assign) CachePolicy cachePolicy;  //  缓存机制
