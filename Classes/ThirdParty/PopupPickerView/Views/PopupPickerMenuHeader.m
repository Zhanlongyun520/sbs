...
+ (UIView *)addHeaderTo:(PopupPickerControl *)control withTitle:(NSString *)title
{
    ...
    if (headView) {
        ...
        UIButton *okBtn = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetWidth(headView.frame) - 60, 0, 60, kHeadH)];
        [okBtn setTitle:@"确定" forState:UIControlStateNormal];
        [okBtn setTitleColor:[UIColor darkTextColor] forState:UIControlStateNormal];
        [okBtn.titleLabel setFont:kHeadFont];
        [okBtn addTarget:control action:@selector(submit) forControlEvents:UIControlEventTouchUpInside];
