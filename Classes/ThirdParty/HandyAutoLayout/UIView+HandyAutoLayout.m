...
- (NSArray *)VFLToConstraints:(NSString *)language
    ...
    NSArray *arr = [NSLayoutConstraint constraintsWithVisualFormat:language
                                                           options:0
                                                           metrics:nil
                                                             views:views];
...
- (NSLayoutConstraint *)constraintCenterYInContainer{
    ...
    return [NSLayoutConstraint constraintWithItem:selfView
                                        attribute:NSLayoutAttributeCenterY
                                        relatedBy:NSLayoutRelationEqual
                                           toItem:selfView.superview
                                        attribute:NSLayoutAttributeCenterY
                                       multiplier:1.0f
                                         constant:00.0f];
...
{
    UIView *selfView = self;
    return [NSLayoutConstraint constraintsWithVisualFormat:@"H:[view]-(left)-[selfView]" options:0 metrics:@{@"left":@(left)} views:NSDictionaryOfVariableBindings(selfView, view)];
}

- (NSArray *)constraintsRight:(CGFloat)right FromView:(UIView *)view
{
    UIView *selfView = self;
    return [NSLayoutConstraint constraintsWithVisualFormat:@"H:[selfView]-(right)-[view]" options:0 metrics:@{@"right":@(right)} views:NSDictionaryOfVariableBindings(view, selfView)];
}
...
- (NSArray *)constraintsCenterEqualToView:(UIView *)view{
    return @[[self constraintCenterXEqualToView:view],
             [self constraintCenterYEqualToView:view]
             ];
}
